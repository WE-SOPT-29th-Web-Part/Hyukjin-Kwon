...
import useInput, { IUseInput, IUseInputWithValidate, IValidInfo } from 'core/useInput';
...
const encodeFileToBase64 = (fileBlob: File): Promise<string | null> => {
  const reader = new FileReader();
  reader.readAsDataURL(fileBlob);
  return new Promise((resolve) => {
    reader.onload = () => {
      if (reader.result instanceof ArrayBuffer) resolve(null);
...
function WritePage() {
  ...
  const titleInput = useInput() as IUseInput;
  const tagInput = useInput() as IUseInput;
  const descInput = useInput() as IUseInput;
  const [fileInput, setFileInput] = useState<File>();
  const handleFile = (e: ChangeEvent) => {
    if (!isHTMLInputElement(e.target) || !e?.target?.files) return;
    setFileInput(e.target.files[0]);
  };

  const { validInfo, ...summary } = useInput(summaryValidate) as IUseInputWithValidate;
  const { isValid, warnText: WarnText } = validInfo as IValidInfo;

  const [tagList, setTagList] = useState<string[]>([]);
  const [isModalOpen, setIsModalOpen] = useState(false);

  const isDuplicateTag = (needCheckTag: string) => new Set(tagList).has(needCheckTag);
  ...
  const removeTag = (targetTag: string) => setTagList(tagList.filter((tag) => tag !== targetTag));
  const publishArticle = async () => {
    if (!validInfo.isValid) return;
    if (summary.value && isAllInputFilled()) {
      let articleInfo: { title: string; summary: string; tags: string[]; thumbnail?: string | null } = {
      ...
      <Modal isOpen={isModalOpen}>
        <ModalContent>
          ...
          <ThumbnailInput
            type="file"
            onChange={(e) => {
              if (e?.target?.files && e.target.files[0].name.match(imageExtRegex)) handleFile(e);
              else e.target.files = null;
            }}
          />
          <Wrapper>
            <SummaryInput {...summary} placeholder="당신의 포스트를 짧게 소개해주세요. (150자 이하)" />
            {!isValid && WarnText && <WarnText />}
          </Wrapper>
          <ButtonWrapper>
            <GrayButton onClick={() => setIsModalOpen(false)}>취소</GrayButton>
            <GreenButton disabled={!validInfo.isValid} onClick={publishArticle}>
              출간하기
            </GreenButton>
          </ButtonWrapper>
        </ModalContent>
